...
$$
\begin{align*}
\text{New Rating} &= w_s \left( w_{sr}\cdot \text{Sentiment}(\text{review}) + \\
&(1-w_{sr}) \text{map}(\text{Sentiment}(\text{review}) - \text{mean}(\text{Sentiment}(\text{review}))) \right) \\
&+ (1-w_s) \left( w_{gr}\cdot \text{Rating}(\text{review}) \\
&+ (1-w_{gr}) \text{map}(\text{Rating}(\text{review}) - \text{mean}(\text{Rating}(\text{review}))) \right)
\end{align*}
$$
